...
void SwLedOn(	struct net_device *dev , PLED_8190 pLed)
{
	...
	RT_TRACE(COMP_LED, "In SwLedOn,LedAddr:%X LEDPIN=%d\n",REG_LEDCFG, pLed->LedPin);
	LedCfg = read_nic_byte(dev, REG_LEDCFG);
	...
	case LED_PIN_LED0:
		write_nic_byte(dev, REG_LEDCFG, (LedCfg&0xf0)|BIT5|BIT6); 
		break;

	case LED_PIN_LED1:
		write_nic_byte(dev, REG_LEDCFG, (LedCfg&0x0f)|BIT5); 
		break;
...
void SwLedOff(struct net_device *dev, PLED_8190 pLed)
{
	...
	RT_TRACE(COMP_LED, "In SwLedOff,LedAddr:%X LEDPIN=%d\n",REG_LEDCFG, pLed->LedPin);
	LedCfg = read_nic_byte(dev, REG_LEDCFG);
	...
	case LED_PIN_LED0:
		...
		if (priv->eeprom_did == 0x8176)
			write_nic_byte(dev, REG_LEDCFG, (LedCfg|BIT3|BIT5|BIT6));
		else
			write_nic_byte(dev, REG_LEDCFG, (LedCfg|BIT3|BIT5));
		break;

	case LED_PIN_LED1:
		LedCfg &= 0x0f; 
		write_nic_byte(dev, REG_LEDCFG, (LedCfg|BIT3));
		break;
...
SwLedBlink8(	PLED_8190 pLed)
{
...
#ifdef MERGE_TO_DO
	if(RT_IS_FUNC_DISABLED(dev, DF_IO_BIT))
	{
	...
	RT_TRACE(COMP_LED, "SwLedBlink8 CurrLedAction %d, \n", pLed->CurrLedState);
	...
	else
	{
		...
		switch( pLed->CurrLedState )
		{
			...
			default:
				RT_ASSERT(false, ("SwLedCm2Blink(): unexpected state!\n"));
				break;
		}		
	}
}
...
void
SwLedBlink9(	PLED_8190 pLed)
{
	struct net_device *dev = (struct net_device *)pLed->dev;
	struct r8192_priv *priv = rtllib_priv(dev);
	bool bStopBlinking = false;
	
	if((priv->rtllib->eRFPowerState == eRfOff) && (priv->rtllib->RfOffReason > RF_CHANGE_BY_PS))
	{
		RT_TRACE( (COMP_LED|COMP_RF), " Hw/Soft Radio Off, turn off Led\n");
		return;
	}

/*	if(RT_IS_FUNC_DISABLED(Adapter, DF_IO_BIT))
	{
		RT_TRACE(COMP_LED, DBG_LOUD, ("Delay LED setting because of IO disable \n"));
	...
	RT_TRACE(COMP_LED, "SwLedBlink9 CurrLedState %d\n", pLed->CurrLedState);
...
void SwLedControlMode8(struct net_device *dev, LED_CTL_MODE LedAction)
{
	struct r8192_priv *priv = rtllib_priv(dev);
...
void SwLedControlMode9(struct net_device *dev, LED_CTL_MODE LedAction)
{
	struct r8192_priv *priv = rtllib_priv(dev);
	PLED_8190 pLed = &(priv->SwLed0);	
...
void LedControl8192CE(struct net_device *dev, LED_CTL_MODE LedAction)
{
	struct r8192_priv *priv = rtllib_priv(dev);
...
#ifdef TO_DO_LIST
	if(Adapter->bInHctTest /*|| Adapter->MgntInfo.NdisVersion==RT_NDIS_VERSION_6_2*/)
		return;

	if(Adapter->bSWInitReady && Adapter->MgntInfo.NdisVersion == RT_NDIS_VERSION_6_2)
	{
		if(GET_PORT_NUMBER(Adapter) == 1)
		{
			RT_TRACE(COMP_LED, DBG_LOUD, ("ignore ext port led action \n"));
			return;
		}	
	}
#endif
