                                        ...
                                        union iwreq_data *wrqu, char *extra)
{
...

	{
		...
		//although WEP40 or WEP104 are all gived as group key in wpa_supplicant,
		//but we always set it pairwise key, 
		//only when pairwise_key_type was not set we can use as group key, because wpa always 
		//set pairwise key first and group key second. 
		//this fix cisco 1242 use wep as group key can not like issue.
		if ((!group) || (IW_MODE_ADHOC == ieee->iw_mode) || ieee->pairwise_key_type == KEY_TYPE_NA)// || (alg ==  KEY_TYPE_WEP40))
		{
		...
		if(wep_nogroup && (ieee->auth_mode !=2))// (!group && (alg & KEY_TYPE_WEP40) && (ieee->auth_mode !=2) )
		{
			if (ext->key_len == 13)
				ieee->pairwise_key_type = alg = KEY_TYPE_WEP104;
			setKey( dev,
