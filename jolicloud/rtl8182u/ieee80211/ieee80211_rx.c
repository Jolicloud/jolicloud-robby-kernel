			...
			u16 stype)
{
	...
	ieee80211_rx_mgt(ieee,skb,rx_stats);	
 ...
 */
static int ieee80211_qos_convert_ac_to_parameters(struct ieee80211_qos_parameter_info *param_elm, 
		struct ieee80211_qos_data *qos_data)
{
        struct ieee80211_qos_ac_parameter *ac_params;
	struct ieee80211_qos_parameters *qos_param = &(qos_data->parameters);
        int rc = 0;
        int i;
	u8 aci;
	u8 acm;
		...
		struct ieee80211_rx_stats *stats)
{
	...
	while (length >= sizeof(*info_element)) {
		...
		case MFIE_TYPE_GENERIC:
			...
			{
				IEEE80211_DEBUG_MGMT("MFIE_TYPE_WZC: %d bytes\n",
						     info_element->len);
	...
	struct ieee80211_rx_stats *stats)
{
	...
	*/
	memset(&network->qos_data, 0, sizeof(struct ieee80211_qos_data));
...
static inline int is_same_network(struct ieee80211_network *src,
				  struct ieee80211_network *dst, u8 ssidbroad)
{
				  ...
				  struct ieee80211_network *src)
{
	...
	if(dst->flags & NETWORK_HAS_QOS_PARAMETERS){
		memcpy(&dst->qos_data, &src->qos_data, sizeof(struct ieee80211_qos_data));
	}
#endif	
...
#ifdef RTL8192U
void InitStaInfo(struct ieee80211_device *ieee,int index)
{
	int idx = index;
...
static u8 IsStaInfoExist(struct ieee80211_device *ieee, u8 *addr)
{
	int k=0;
	struct sta_info * psta = NULL;
...
static u8 GetFreeStaInfoIdx(struct ieee80211_device *ieee, u8 *addr)
{
	int k = 0;
...
struct sta_info *GetStaInfo(struct ieee80211_device *ieee, u8 *addr)
{
	int k=0;
	struct sta_info * psta = NULL;
...
void IbssAgeFunction(struct ieee80211_device *ieee)
{
	...
	if(is_beacon(beacon->header.frame_ctl)){
		if(ieee->iw_mode == IW_MODE_ADHOC)
		{
				...
				&& (ieee->current_network.channel <= 14))
			{
				...
				{
					int idx = 0;
					struct ieee80211_hdr_3addr* header = NULL;
					int idx_exist = 0;
					if(timer_pending(&ieee->ibss_wait_timer))
						del_timer_sync(&ieee->ibss_wait_timer);
					header = (struct ieee80211_hdr_3addr*)&(beacon->header);
					...
					} else 
					{
						...
						queue_delayed_work(ieee->wq, &ieee->update_assoc_sta_info_wq, 0);
#elif LINUX_VERSION_CODE < KERNEL_VERSION(2,5,0)
						schedule_task(&ieee->update_assoc_sta_info_wq);
#else
						queue_work(ieee->wq,&ieee->update_assoc_sta_info_wq);
#endif
				...
				else
				{
					...
					u8CurrTsf = stats->TimeStampHigh;
					u8CurrTsf <<= 32;
					u8CurrTsf |= stats->TimeStampLow;

					if(u8CurrTsf < bssDesc.bdTstamp)
					{
						printk(" %s():Merge two IBSSs. \n",__FUNCTION__);
						
						JoinRequest( Adapter, pMgntInfo->JoinAction, &bssDesc );
					}
#endif
...
void ieee80211_rx_mgt(struct ieee80211_device *ieee,
		      ...
		      struct ieee80211_rx_stats *stats)
{
    struct ieee80211_hdr_4addr *header = (struct ieee80211_hdr_4addr *)skb->data ;
#if 0
    ...
    switch (WLAN_FC_GET_STYPE(header->frame_ctl)) {
        ...
        case IEEE80211_STYPE_PROBE_REQ:
            IEEE80211_DEBUG_MGMT("received PROBE RESQUEST (%d)\n",
                    WLAN_FC_GET_STYPE(header->frame_ctl));
...
#ifdef RTL8192U
EXPORT_SYMBOL(IbssAgeFunction);
EXPORT_SYMBOL(GetStaInfo);
#endif
#else
EXPORT_SYMBOL_NOVERS(ieee80211_rx_mgt);
EXPORT_SYMBOL_NOVERS(ieee80211_rx);
#ifdef RTL8192U
EXPORT_SYMBOL_NOVERS(IbssAgeFunction);
EXPORT_SYMBOL_NOVERS(GetStaInfo);
#endif
#endif
#endif
