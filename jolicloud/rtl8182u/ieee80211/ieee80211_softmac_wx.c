...
void ieee80211_wx_sync_scan_wq(struct ieee80211_device *ieee)
{
	...
	if (ieee->SetFwCmdHandler)
	{
		if(ieee->pHTInfo->IOTAction & HT_IOT_ACT_MID_HIGHPOWER)
			ieee->SetFwCmdHandler(ieee->dev, FW_CMD_MID_HIGH_PWR_ENABLE);
		else
				 ...
				 union iwreq_data *wrqu, char *extra)
{
	...
	if(
		...
		IEEE80211_DEBUG(IEEE80211_DL_ERR,"%s(): PS mode is tryied to be use but driver missed a callback\n\n",__FUNCTION__);	
