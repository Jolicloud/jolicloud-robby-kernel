...
static struct sk_buff* rtllib_probe_resp(struct rtllib_device *ieee, u8 *dest)
{
	...
	if ((ieee->iw_mode == IW_MODE_MASTER) && (ieee->bhidden_ssid == 1)) {
		printk("\nHidden SSID set in Beacon!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
		ssid = NULL;
		ssid_len = 0;
	}
	...
	if (ieee->iw_mode == IW_MODE_MASTER)
		beacon_buf->capability = 
			cpu_to_le16(ieee->current_network.capability & WLAN_CAPABILITY_ESS);
	else
	beacon_buf->capability = 
	...
	*(tag++) = ieee->current_network.channel;
	...
	if (tim_len) {
		*(tag++) = MFIE_TYPE_TIM;
		*(tag++) = tim_len;
		memcpy(tag, ieee->Tim.Octet, tim_len);
		tag+=tim_len;
	}
#endif
...
inline struct sk_buff *rtllib_association_req(struct rtllib_network *beacon,struct rtllib_device *ieee)
{
	...
	if ((ieee->rtllib_ap_sec_type && 
		...
		(ieee->pHTInfo->IOTPeer != HT_IOT_PEER_RALINK) && 
		(ieee->pHTInfo->IOTPeer != HT_IOT_PEER_BROADCOM))) || 
...
void rtllib_start_master_bss(struct rtllib_device *ieee)
{
	...
	ieee->current_network.capability = WLAN_CAPABILITY_ESS;
#endif
#ifndef ASL
	ieee->set_chan(ieee->dev, ieee->current_network.channel);
#endif
