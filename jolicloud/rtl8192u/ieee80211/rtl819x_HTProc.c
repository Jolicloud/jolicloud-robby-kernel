...
bool HTIOTActIsDisableMCSTwoSpatialStream(struct ieee80211_device* ieee)
{
	...
	if(ieee->is_ap_in_wep_tkip && ieee->is_ap_in_wep_tkip(ieee->dev))
	{
        ...
        if(ieee->is_ap_in_wep_tkip && ieee->is_ap_in_wep_tkip(ieee->dev))
        {
            if(ieee->ieee80211_ap_sec_type != NULL) 
                if(ieee->ieee80211_ap_sec_type(ieee->dev) == SEC_ALG_TKIP){
...
u8 HTIOTActIsMgntUseCCK6M(struct ieee80211_device* ieee,struct ieee80211_network *network)
{
	u8	retValue = 0;
...
u8
HTIOTActWAIOTBroadcom(struct ieee80211_device* ieee)
{
	PRT_HIGH_THROUGHPUT	pHTInfo = ieee->pHTInfo;
	u8		retValue = false;
	u8		boundary=59;
...
u8 HTIOTActIsCCDFsync(struct ieee80211_device *ieee)
{
	u8	retValue = 0;
  ...
  HTIOTActIsEDCABiasRx(struct ieee80211_device* ieee,struct ieee80211_network *network)
{
	...
	{
            if(ieee->ieee80211_ap_sec_type != NULL) 
                if(ieee->ieee80211_ap_sec_type(ieee->dev) == SEC_ALG_CCMP)
                    if(pHTInfo->IOTPeer==HT_IOT_PEER_RALINK){
...
bool HTIOCActIsDisableCckRate(struct ieee80211_device* ieee,struct ieee80211_network *network)
{
	bool 	retValue = false;
#if defined(RTL8192SU)
	PRT_HIGH_THROUGHPUT	pHTInfo = ieee->pHTInfo;
...
EXPORT_SYMBOL_NOVERS(HTFilterMCSRate);
EXPORT_SYMBOL_NOVERS(HTGetHighestMCSRate);
EXPORT_SYMBOL_NOVERS(MCS_FILTER_ALL);
EXPORT_SYMBOL_NOVERS(MCS_FILTER_1SS);
#else
EXPORT_SYMBOL(HTUpdateSelfAndPeerSetting);
EXPORT_SYMBOL(HTFilterMCSRate);
EXPORT_SYMBOL(HTGetHighestMCSRate);
EXPORT_SYMBOL(MCS_FILTER_ALL);
EXPORT_SYMBOL(MCS_FILTER_1SS);
#endif
#endif
