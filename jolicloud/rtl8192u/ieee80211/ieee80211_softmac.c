...
{
	struct delayed_work *dwork = container_of(work,struct delayed_work,work);
        struct ieee80211_device *ieee = container_of(dwork, struct ieee80211_device, associate_procedure_wq);
#else
			...
			*/
			if ((ieee->softmac_features & IEEE_SOFTMAC_ASSOCIATE) &&
				...
				if(ieee->LedControlHandler != NULL)
				        ieee->LedControlHandler(ieee->dev, LED_CTL_START_TO_LINK); 
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,5,0)					
				queue_delayed_work(ieee->wq, &ieee->associate_procedure_wq, 5); 
#else
				schedule_task(&ieee->associate_procedure_wq);
#endif
			}
...
void ieee80211_softmac_stop_protocol(struct ieee80211_device *ieee,bool shutdown)
{
	ieee->sync_scan_hurryup = 1; 
	down(&ieee->wx_sem);
...
void ieee80211_stop_protocol(struct ieee80211_device *ieee,bool shutdown)
{
	if (!ieee->proto_started)
		return;
...
void ieee80211_softmac_init(struct ieee80211_device *ieee)
{
...
#ifdef RTL8192U
	init_timer(&ieee->ibss_wait_timer);
	ieee->ibss_wait_timer.data = (unsigned long) ieee;
	ieee->ibss_wait_timer.function = ieee80211_ibss_wait_timeout;
#endif
	...
	ieee->wq = create_workqueue(DRV_NAME);
        ...
        INIT_DELAYED_WORK(&ieee->associate_procedure_wq, ieee80211_associate_procedure_wq);
...
EXPORT_SYMBOL(ieee80211_sta_ps_send_pspoll_frame);
EXPORT_SYMBOL(ieee80211_sta_wakeup);
#else
...
EXPORT_SYMBOL_NOVERS(ieee80211_sta_wakeup);
#endif
#endif
