...
#ifdef RTL8192U
		static u32 edca_setting_DL[HT_IOT_PEER_MAX] = 
		{ 0x5e4322, 	0x5e4322, 	0x5ea44f,		0x5e4322, 	0x604322, 	0xa44f, 		0xa630, 		0xa42b,		0x5ea44f,	0x5e4322};
		static u32 edca_setting_UL[HT_IOT_PEER_MAX] = 
		{ 0x5e4322, 	0xa44f, 		0x5ea44f,		0x5e4322, 	0x604322, 	0x5ea44f, 	0x3ea44f,	0x5ea42b,	0x5ea44f,	0x5e4322};
#endif
...
static	void	dm_initial_gain_BeforeConnect(struct net_device *dev);
...
void dm_CheckRxAggregation(struct net_device *dev)
{
	...
	if (priv->bForcedUsbRxAggr)
	{
		if (priv->ForcedUsbRxAggrInfo == 0)
		{
			...
			{
				priv->ieee80211->HalUsbRxAggrHandler(dev, false);
			}
		}
		else
		{
			...
			{
				priv->ieee80211->HalUsbRxAggrHandler(dev, true);
			}
		}
		return;
	}
...
static void dm_TXPowerTrackingCallback_ThermalMeter_92S(struct net_device * dev)
{
	struct r8192_priv *priv = ieee80211_priv(dev);
	...
	if(ThermalValue)
	{
		...
		{
			priv->ieee80211->SetFwCmdHandler(dev, FW_CMD_TXPWR_TRACK_THERMAL);	
		}
		else
		{
...
static void dm_CheckTXPowerTracking_ThermalMeter(struct net_device *dev)
{
	...
	{
#ifdef RTL8192SU
		dm_TXPowerTrackingCallback_ThermalMeter_92S(dev);
#else
		#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,20)
			queue_delayed_work(priv->priv_wq,&priv->txpower_tracking_wq,0);
		#else
...
/*-----------------------------------------------------------------------------
 * Function:    FalseAlarmCounterStatistics()
 *
 * Overview:    function for statistics the false alarm counters
 *
 ...
 *
 * Note:		
 *	03/04/2009	porting from 9x series
 *---------------------------------------------------------------------------*/
void dm_FalseAlarmCounterStatistics(
	struct net_device *dev)
{
	u32 ret_value;
	struct r8192_priv *priv = ieee80211_priv(dev);
	...
	/*
	rtl8192_QueryBBReg(dev, rCCK0_FalseAlarmReport, bPHYCounterReset, 1);
	
	ret_value = rtl8192_QueryBBReg(dev, rCCK0_FACounterLower, bCCKFalseAlarmCounterLow);
	FalseAlmCnt->Cnt_Cck_fail = ret_value;

	ret_value = rtl8192_QueryBBReg(dev, rCCK0_DSPReport, bCCKFalseAlarmCounterHigh);
	...
	*/
	...
	/*
	PHY_SetBBReg(Adapter, rOFDM1_LSTF, bOFDMCounterReset, 1);
	PHY_SetBBReg(Adapter, rOFDM1_LSTF, bOFDMCounterReset, 0);
	PHY_SetBBReg(Adapter, rCCK0_RxDescriptor, bCCKCounterReset, 0);
	PHY_SetBBReg(Adapter, rCCK0_RxDescriptor, bCCKCounterEnable, 1);
	*/
...
static void dm_CtrlInitGainBeforeConnectByRssiAndFalseAlarm(
	struct net_device *dev)
{
	struct r8192_priv *priv = ieee80211_priv(dev);
	...
	if(priv->ieee80211->be_scan_inprogress)
		return;

	if((priv->ieee80211->state >= IEEE80211_ASSOCIATING) && (priv->ieee80211->state >= IEEE80211_ASSOCIATING_AUTHENTICATED))
	{
		dm_digtable.cur_connect_state = DIG_BEFORE_CONNECT;
	}
	...
	dm_initial_gain_BeforeConnect(dev);
	...
	struct net_device * dev)
{
	...
	if(priv->ieee80211->be_scan_inprogress == true)
	{
		force_write = 1;
		return;
	}
...
static void dm_initial_gain_BeforeConnect(
	struct net_device * dev)
{
	struct r8192_priv *priv = ieee80211_priv(dev);
	u8			initial_gain=0;
		...
		(dm_digtable.cur_connect_state == DIG_BEFORE_CONNECT) )
	{
		...
		if(dm_digtable.cur_connect_state == DIG_BEFORE_CONNECT)
		{
			...
			if(priv->FalseAlmCnt.Cnt_all > 16000)
				dm_digtable.cur_ig_value = dm_digtable.rx_gain_range_max;
#if 0
			if (DM_DigTable.Rssi_val >= 25)
				DM_DigTable.InitialGainLowerBoundState = FALSE;
			else if ((DM_DigTable.Rssi_val <= 20))
				DM_DigTable.InitialGainLowerBoundState = TRUE;
			if(DM_DigTable.InitialGainLowerBoundState)
			{
				DM_DigTable.CurIGValue = DM_DigTable.rx_gain_range_min;
				DbgPrint("Lower bound state !!\n");
			}
#endif
		}
		else 
	...
	{		
		ret_value = read_nic_byte(dev, 0x364);
		if( !(ret_value & BIT0) )
		{
			ret_value = ret_value | BIT0;
			write_nic_byte(dev, 0x364, ret_value);
		}
...
#if 0
	if(pAdapter->ResetCount != reset_cnt)
	{
		force_write = 1;
		reset_cnt = pAdapter->ResetCount;
	}
#endif
...
static void dm_ctstoself(struct net_device *dev)
{
	...
	if(pHTInfo->IOTPeer == HT_IOT_PEER_BROADCOM)
	{
		...
		else	
		{
			...
			pHTInfo->IOTAction |= HT_IOT_ACT_FORCED_CTS2SELF;
		#else
			if(priv->undecorated_smoothed_pwdb < priv->ieee80211->CTSToSelfTH)	
			{
				pHTInfo->IOTAction &= ~HT_IOT_ACT_FORCED_CTS2SELF;
			}
			else if(priv->undecorated_smoothed_pwdb >= (priv->ieee80211->CTSToSelfTH+5))	
			{
				pHTInfo->IOTAction |= HT_IOT_ACT_FORCED_CTS2SELF;
			}
		#endif
	...
	for(i=0;i<PEER_MAX_ASSOC; i++)
	{
		...
		if(NULL != pEntry)
		{
			...
			if((pEntry->wireless_mode!=WIRELESS_MODE_N_24G) && (pEntry->wireless_mode != WIRELESS_MODE_N_5G))
				continue;
			bShortGIEnabled = (pEntry->htinfo.bCurTxBW40MHz && priv->ieee80211->pHTInfo->bCurShortGI40MHz && pEntry->htinfo.bCurShortGI40MHz) |
				(!pEntry->htinfo.bCurTxBW40MHz && priv->ieee80211->pHTInfo->bCurShortGI20MHz && pEntry->htinfo.bCurShortGI20MHz);
			pRA->upper_rssi_threshold_ratr =
