...
#include "nv-map-impl.h"

#else /* NV_USER_MAP */

#define NV_DEV_NAME "nvidia"
#define nv_user_map_init() 
#define nv_register_ctl_dev() (0) 
#define nv_unregister_ctl_dev() (0)
#define nv_register_bars(a) (0)
#define nv_unregister_bars(a)
...
static void nv_check_pci_config_space(nv_state_t *nv, BOOL check_the_bars)
{
...
static u8 nv_find_pci_capability(struct pci_dev *dev, u8 capability)
{
    u16 status;
    u8  cap_ptr, cap_id;
...
#ifdef NV_USER_MAP
    struct proc_dir_entry *proc_driver;
#endif /* NV_USER_MAP */
...
#ifdef NV_USER_MAP
    proc_driver = create_proc_entry("driver", d_flags, NULL);
    if (!proc_driver)
        goto failed;
    proc_nvidia = create_proc_entry("nvidia", d_flags, proc_driver);
    if (!proc_nvidia)
        goto failed;
#else /* NV_USER_MAP */
...
static int __init nvidia_init_module(void)
{
    ...
    memset(&nv_smu_device, 0, sizeof(nv_state_t));
    ...
    rc = register_chrdev(nv_major, NV_DEV_NAME, &nv_fops);
    if (rc < 0)
    {
    ...
    for (nvl = nv_linux_devices; nvl != NULL;  nvl = nvl->next)
    {
         ...
         */
        NV_ATOMIC_SET(nvl->tasklet.count, 1);
    }
...
failed:
    ...
    (void) nv_unregister_ctl_dev();

    unregister_chrdev(nv_major, NV_DEV_NAME);
...
#if defined(NVCPU_X86_64) && defined(CONFIG_IA32_EMULATION) && !defined(HAVE_COMPAT_IOCTL)
    /* Unregister ioctl()'s for 32-bit clients */
...
)
{
    ...
    if ( ! (nv->flags & NV_FLAG_OPEN))
    {
        ...
        if (!NV_IS_GVI_DEVICE(nv))
        {
            rm_read_registry_dword(sp, nv, "NVreg", NV_REG_ENABLE_MSI, &msi_config);
            if ((msi_config == 1) && 
                (nv_find_pci_capability(nvl->dev, PCI_CAP_ID_MSI)))
            {
                ...
                {
                    nv->flags &= ~NV_FLAG_USES_MSI;              
                    nv_printf(NV_DBG_ERRORS, "NVRM: failed to enable MSI, \n");
                    nv_printf(NV_DBG_ERRORS, "using PCI-E virtual-wire interrupts!\n");
                }
            }
         }
#endif
        ...
        if (NV_IS_GVI_DEVICE(nv))
        {
            ...
            if (rc == 0)
            {
                NV_TASKQUEUE_INIT(&nvl->work, nv_gvi_kern_bh, nvl);
...
#if defined(NV_XEN_SUPPORT_OLD_STYLE_KERNEL) && defined(NVCPU_X86)
...
#ifdef NV_USE_TASKLET_INIT
        tasklet_init(&nvl->tasklet,
                     nv_kern_isr_bh,
                     (unsigned long) nv);
#else /* NV_USE_TASKLET_INIT */
        nvl->tasklet.func = nv_kern_isr_bh;
        nvl->tasklet.data = (unsigned long) nv;
        tasklet_enable(&nvl->tasklet);
#endif /* NV_USE_TASKLET_INIT */
...
)
{
    ...
    rm_free_unused_clients(sp, nv, (void *)file);

    NV_RELEASE_USERMAP_LIST(nv);
...
)
{
    ...
    else // if (NV_VMA_OFFSET(vma) == NV_MMAP_ALLOCATION_OFFSET)
    {
        ...
        nv_printf(NV_DBG_INFO,
                  "NVRM: %s: Setting VM_DRIVER_PAGES and remapping (vma: 0x%p)"
                  " %d system pages, index %d, for 'at' 0x%p\n",
                  __FUNCTION__, vma, pages, i, at);
#endif
    }
    ...
    unsigned long i_arg)
{
    ...
    switch (arg_cmd)
    {
        ...
        default:
            status = rm_ioctl(sp, nv, file, arg_cmd, arg_copy)
                        ? 0 : -EINVAL;
            break;
    }
        ...
        int count, int *eof, void *data)
{
    ...
    U032 fpga_rev1, fpga_rev2, fpga_rev3;
    nv_stack_t *sp = NULL;
    ...
    *eof = 1;
    ...
    {
        if (rm_get_device_name(sp, nv, dev->device, NV_DEVICE_NAME_LENGTH,
                               tmpstr) != RM_OK) {
            strcpy (tmpstr, "Unknown");
        }
    }
...
)
{
    ...
    at->key_mapping = (void *) ((NvUPtr) (at->page_table[0]->dma_addr));
...
failed:
    ...
    nv_printf(NV_DBG_ERRORS, "NVRM: filed to unmap user pages at 0x%llx\n",
              address);
    return 0;
}
#endif /* NV_USER_MAP */
...
NvU64 NV_API_CALL nv_get_kern_phys_address(NvU64 address)
{
    ...
    if ((address > PAGE_OFFSET) && (address < VMALLOC_START))
        return __pa(address);
#endif /* (! defined(NV_NO_PAGE_STRUCT)) */
...
{
    nv_alloc_t *at;
    RM_STATUS rm_status = RM_ERR_NO_FREE_MEM;
    nv_linux_state_t *nvl = NV_GET_NVL_FROM_NV_STATE(nv);
    nv_stack_t *sp = NULL;
    U032 i, memory_type;
    ...
    if (agp_memory)
    {
        ...
        {
            rm_status = RM_ERR_NOT_SUPPORTED;
            goto failed;
        }
        ...
        if (NV_OSAGP_ENABLED(nv))
        {
            ...
            rm_status = KernAllocAGPPages(sp, nv, page_count, priv_data, &offset);
            if (rm_status != RM_OK)
                goto failed;

            KernLoadAGPPages(nv, at, *priv_data);
        } else {
            rm_status = rm_alloc_agp_pages(sp, nv, page_count, priv_data, &offset);
            if (rm_status != RM_OK)
                goto failed;
        }
...
)
{
    ...
    if (!(flags & NV_FLAG_GVI))
    {
        ...
        {
            nv_printf(NV_DBG_ERRORS, "NVRM: ignoring the legacy GPU %02x:%02x.%x\n",
    ...
    for (i = 0; i < (NV_GPU_NUM_BARS - 1); i++)
    {
        ...
        nv_printf(NV_DBG_ERRORS,
            ...
            "NVRM: BAR%d is %dM @ 0x%llx (PCI:%04x:%02x.%x)\n", i,
    ...
    else
    {
        ...
        if (nv_ext_kern_probe(num_nv_devices))
        {
            nv_printf(NV_DBG_ERRORS, "NVRM: failed extended probe!!\n");
            goto err_zero_dev;
        }
    }

    nv_printf(NV_DBG_INFO,
            "NVRM: PCI:%02x:%02x.%x (%04x:%04x): BAR0 @ 0x%llx (%lluMB)\n",
            nv->bus, nv->slot, PCI_FUNC(dev->devfn),
            nv->vendor_id, nv->device_id, nv->regs->address,
            (nv->regs->size >> 20));
    nv_printf(NV_DBG_INFO,
            "NVRM: PCI:%02x:%02x.%x (%04x:%04x): BAR1 @ 0x%llx (%lluMB)\n",
            nv->bus, nv->slot, PCI_FUNC(dev->devfn),
...
)
{
    ...
    {
        nv_printf(NV_DBG_INFO, "NVRM: ignoring the SMU %02x:%02x.%x\n",
                  NV_PCI_BUS_NUMBER(dev), NV_PCI_SLOT_NUMBER(dev),
                  PCI_FUNC(dev->devfn));
        goto failed;
    }
    ...
    if (!NV_PCI_RESOURCE_VALID(dev, 0))
    {
        nv_printf(NV_DBG_INFO,
            ...
            "NVRM: BAR0 is %dM @ 0x%08x (PCI:%04x:%02x.%x)\n", 
    ...
    nv_printf(NV_DBG_INFO, "NVRM: %02x:%02x.%x %04x:%04x - 0x%08x [size=%dK]\n",
            nv->bus, nv->slot, PCI_FUNC(dev->devfn),
            nv->vendor_id, nv->device_id, nv->regs->address,
            nv->regs->size / (1024));
